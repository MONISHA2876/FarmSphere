...
function SoilPredictor() {
    ...
    try {
      ...
      if (res.data.error || res.data.message === "Error analyzing image") {
        setResult({
          ...
          confidence: 0,
          notes:
            "The soil analysis AI is being updated for better compatibility. Our team is working to resolve this issue. Please try again in a few minutes, or contact support if the problem persists.",
          crops: [],
          care: [],
        });
      } else {
      ...
      setResult({
        ...
        confidence: 0,
        notes:
          "Unable to analyze the image. Please try again with a clearer image or check your connection.",
        crops: [],
        care: [],
      });
      setAnimateResult(true);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-green-100 py-12 px-4 sm:px-6">
      ...
      <div className="max-w-3xl mx-auto mb-6"></div>
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden transform transition-all duration-300 hover:shadow-2xl">
        <div className="bg-smart-green text-white py-6 px-8 rounded-t-2xl">
          ...
          <p className="mt-2 text-green-100 opacity-90">
            Upload a soil image for instant analysis and recommendations
          </p>
        </div>
        ...
        <div className="p-8">
            ...
            {!preview ? (
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className={`w-full bg-smart-yellow text-smart-green font-bold px-6 py-3 rounded-lg transform transition hover:scale-105 ${
                    isLoading
                ...
                >
                  {isLoading ? (
                    <span className="flex items-center justify-center">
                      <svg
                        className="animate-spin -ml-1 mr-3 h-5 w-5 text-smart-green"
          ...
          {result && (
            ...
            >
              <div className="flex items-center justify-between">
                <h3
                  className={`text-2xl font-bold ${
                    result.error ? "text-red-600" : "text-smart-green"
                  }`}
                >
              ...
              <div
                className={`mt-4 p-4 ${
                  result.error ? "bg-red-100 border border-red-200" : "bg-white"
                ...
                <div
                  className={`mt-4 p-4 ${
                    result.error
                      ...
                      : "bg-white"
                  } rounded-lg shadow-inner transition-all duration-300 hover:shadow-md`}
                >
                  <h4
                    className={`font-bold ${
                      result.error ? "text-red-700" : "text-gray-700"
                    }`}
                  >
                  ...
                  {result.error && (
                    <div className="mt-3 p-3 bg-yellow-100 border border-yellow-300 rounded-lg">
                      ...
                      <div className="mt-2 text-yellow-700 text-sm space-y-2">
                        <p>
                          <strong>Current Status:</strong> The soil analysis AI
                          model is being updated for better compatibility with
                          our servers.
                        </p>
                        <p>
                        ...
                        <ul className="ml-4 space-y-1">
                          <li>
                            • Try the Plant Disease Detection feature instead
                          </li>
                          <li>
                            • Check the Market Prediction for crop pricing
                          </li>
                        ...
                        <p className="mt-2 text-xs text-yellow-600">
                          <strong>Technical note:</strong> We're resolving
                          TensorFlow model compatibility issues on our servers.
                        </p>
                      </div>
                    </div>
                  )}
                      ...
                      <ul className="mt-3 space-y-2">
                        {result.care.map((tip, index) => (
                          <li
                            ...
                            style={{ animationDelay: `${index * 100}ms` }}
                          >
                            <span className="mr-2 text-smart-yellow">•</span>{" "}
                            {tip}
                          </li>
                        ))}
                      </ul>
